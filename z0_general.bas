Attribute VB_Name = "z0_general"
Public Function StringsShareWords(ByVal input1 As String, ByVal input2 As String, ByRef diffs As Scripting.Dictionary) As Boolean
    ...
    Dim trimmer As WordStemmer
    Set trimmer = New WordStemmer
    
    Dim s1 As String, s2 As String
    Dim dict1 As Scripting.Dictionary
    Dim dict2 As Scripting.Dictionary
    Dim full_keys As Scripting.Dictionary
    
    Set dict1 = New Scripting.Dictionary
    Set dict2 = New Scripting.Dictionary
    Set full_keys = New Scripting.Dictionary
    
    s1 = Trim(LCase$(input1))
    s2 = Trim(LCase$(input2))
    ...
    Dim store_diffs As Boolean
    If diffs Is Nothing Then
        store_diffs = False
    Else
        store_diffs = True
    End If
    ...
    For i = 0 To UBound(arr1)
        ...
        tmp_str = trimmer.porterAlgorithm(arr1(i))
        If (tmp_str <> "") Then
            If dict1.Exists(tmp_str) = False Then
                dict1.Add tmp_str, tmp_str
            End If
        End If
    Next
    
    For i = 0 To UBound(arr2)
        tmp_str = trimmer.porterAlgorithm(arr2(i))
        If (tmp_str <> "") Then
            If dict2.Exists(tmp_str) = False Then
                dict2.Add tmp_str, tmp_str
                full_keys.Add tmp_str, arr2(i) 'do this only for the 2nd input for now, which is the reference string
            End If
        End If
    Next
    ...
    For i = 0 To UBound(dict1.Keys)
        If (dict1.Keys(i) <> "") Then
            If dict2.Exists(dict1.Keys(i)) = True Then
                match_found = True
                If store_diffs = False Then Exit For
            Else
                'store the word
                'If store_diffs Then
                    'If diffs.Exists(dict1.keys(i)) = False Then diffs.Add dict1.keys(i), dict1.keys(i)
                'End If
            End If
        End If
    Next
    
    For i = 0 To UBound(dict2.Keys)
        If (dict2.Keys(i) <> "") Then
            If dict1.Exists(dict2.Keys(i)) = True Then
                match_found = True
                If store_diffs = False Then Exit For
            Else
                If store_diffs And diffs.Exists(dict2.Keys(i)) = False Then
                    diffs.Add full_keys(dict2.Keys(i)), full_keys(dict2.Keys(i)) ''do this only for the 2nd input for now, which is the reference string
                End If
            End If
        End If
    Next
        
    StringsShareWords = match_found
End Function
